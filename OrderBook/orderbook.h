...
class Orderbook_level_Snapshot{
    public:
    Orderbook_level_Snapshot(const Level_Infos &bids,const Level_Infos &ask);
    const Level_Infos& Get_Bids()const;
    const Level_Infos& Get_Ask() const;
    private:
    Level_Infos bids_;
    Level_Infos ask_;
};
...
class Order{
    ...
    // code of contructor is not needed 
